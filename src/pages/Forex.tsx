...
const Forex = () => {
  ...
  const fetchForexData = async () => {
    try {
      ...
      const response = await fetch(
        `https://financialmodelingprep.com/api/v3/quotes/forex?apikey=${import.meta.env.VITE_FMP_API_KEY}`
      )
  ...
  const fetchForexDetail = async (symbol: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to fetch forex detail')
      const data = await res.json()
      if (Array.isArray(data) && data.length > 0) {
        setSelectedForex(data[0])
      } else {
        setDetailError('No detail found for this forex pair.')
      }
    } catch (err) {
  ...
  const handleTimeRangeChange = (range: TimeRange) => {
    ...
    if (selectedForex) {
      fetchDailyData(selectedForex.symbol, range)
    }
  }
  ...
  const handleSuggestionClick = (f: ForexData) => {
    setSearchQuery(f.symbol)
    setShowSuggestions(false)
    setSelectedForex(f)
    fetchForexDetail(f.symbol)
    fetchIntradayData(f.symbol)
    fetchDailyData(f.symbol, selectedTimeRange)
  }

  return (
    <div className="container mx-auto mt-6">
      <h2 className="text-3xl font-bold mb-8">Forex Market</h2>
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-8" ref={searchRef}>
        ...
        {selectedForex && !detailLoading && !detailError && (
          <div className="mt-6 space-y-6">
            <div className="bg-gray-50 rounded-lg p-4 shadow-inner">
              <div className="flex items-center gap-4 mb-2">
                <div className="text-xl font-bold">{selectedForex.name} ({selectedForex.symbol})</div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                <div>Current Price: <span className="font-semibold">{selectedForex.price?.toFixed(4)}</span></div>
                <div>Change %: <span className={`font-semibold ${selectedForex.changesPercentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>{selectedForex.changesPercentage?.toFixed(2)}%</span></div>
                <div>Open: <span className="font-semibold">{selectedForex.open?.toFixed(4) ?? '-'}</span></div>
                <div>High: <span className="font-semibold">{selectedForex.dayHigh?.toFixed(4) ?? '-'}</span></div>
                <div>Low: <span className="font-semibold">{selectedForex.dayLow?.toFixed(4) ?? '-'}</span></div>
                <div>Prev Close: <span className="font-semibold">{selectedForex.previousClose?.toFixed(4) ?? '-'}</span></div>
                <div>Bid: <span className="font-semibold">{selectedForex.bid?.toFixed(4) ?? '-'}</span></div>
                <div>Ask: <span className="font-semibold">{selectedForex.ask?.toFixed(4) ?? '-'}</span></div>
                <div>Volume: <span className="font-semibold">{selectedForex.volume?.toLocaleString() ?? '-'}</span></div>
              </div>
            </div>
            {/* Price Chart */}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full rounded-xl overflow-hidden">
              <thead className="bg-gray-100 sticky top-0 z-10">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-600 uppercase">Pair</th>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-bold text-gray-600 uppercase">Bid</th>
                  <th className="px-6 py-3 text-right text-xs font-bold text-gray-600 uppercase">Ask</th>
              ...
              <tbody>
                {forexData.map((forex, idx) => (
                  ...
                  >
                    <td className="px-6 py-3 font-bold">{forex.symbol}</td>
                    <td className="px-6 py-3">{forex.name}</td>
                    <td className="px-6 py-3 text-right">{forex.price?.toFixed(4)}</td>
                    <td className={`px-6 py-3 text-right font-semibold ${
                      forex.changesPercentage >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {forex.changesPercentage?.toFixed(2)}%
                    </td>
                    <td className="px-6 py-3 text-right">{forex.open?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.dayHigh?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.dayLow?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.previousClose?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.bid?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.ask?.toFixed(4) ?? '-'}</td>
                    <td className="px-6 py-3 text-right">{forex.volume ?? '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
